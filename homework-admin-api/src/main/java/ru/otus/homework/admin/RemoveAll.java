package ru.otus.homework.admin;
...
@SuppressWarnings({"java:S1192", "java:S112", "java:S1144", "java:S125"})
public class RemoveAll {
    ...
    private static Collection<String> sync(Admin client) throws Exception {
        ...
        client.deleteTopics(topics).all().get(); // а вот еще раз
    ...
    private static void async2(Admin client) throws Exception {
        client.listTopics()
                ...
                .whenComplete(
                        (ignored, ex) -> {
                            if (ex == null) Utils.log.info("SUCCESS");
                            else Utils.log.error("Troubles...", ex);
                        })
                .toCompletableFuture()
                .get();
    }

    private static void async3(Admin client) {
        Mono.fromCompletionStage(client.listTopics().listings().toCompletionStage())
                .map(
                        listings ->
                                listings.stream()
                                        .filter(it -> !it.isInternal())
    ...
    public static void removeAll(Admin client) throws Exception {
        ...
        // не нашел лучшего варианта. После удаления на сомом деле топики еще не удалены и создать
        // их нельзя
